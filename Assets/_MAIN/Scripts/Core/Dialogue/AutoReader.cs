...
namespace DIALOGUE
{
    ...
    {
        private const int DEFAULT_CHARACTERS_READ_PER_SECOND = 20;
        private const float READ_TIME_PADDING = 0.5f;
        private const float MIN_READ_TIME = 1f;
        private const float MAX_READ_TIME = 99f;
        private const string STATUS_TEXT_AUTO = "Авто";
        private const string STATUS_TEXT_SKIP = "Пропуск";
        ...
        private IEnumerator AutoRead()
        {
            ...
            if (!architect.isBuilding && architect.currentText != string.Empty)
                DialogueSystem.instance.OnUserSystem_Next();

            while (conversationManager.isRunning)
            {
                if(!skip)
                {
                    while(!architect.isBuilding && !conversationManager.isWaitingForAutoReader)
                        yield return null;
